...
class ArchiveStore(context: Context) : SQLiteOpenHelper(context, "tg_native_index.db", null, 1) {
    ...
    fun search(q: String, chat: Long? = null): List<LocalMessage> { val where=mutableListOf<String>(); val args=mutableListOf<String>(); if(q.isNotBlank()){where+="normalized LIKE ?";args+="%${q.lowercase(Locale.ROOT)}%"};if(chat!=null){where+="chat_id=?";args+=chat.toString()}; return readableDatabase.query("messages",null,where.joinToString(" AND ").ifBlank{null},args.toTypedArray(),null,null,"date DESC","250").use{c->rows(c)} }
    ...
    fun addDemo():Int = insert(listOf(LocalMessage(remoteId=1,chatId=100,chatName="演示会话 A",sender="示例用户",date="2026-01-01T09:30:00Z",text="这是匿名演示内容，仅用于验证本地索引。"),LocalMessage(remoteId=2,chatId=100,chatName="演示会话 A",sender="示例用户",date="2026-01-01T10:00:00Z",text="API 参数、验证码和两步验证密码不应写入应用数据库或日志。"),LocalMessage(remoteId=3,chatId=200,chatName="演示会话 B",sender="示例用户",date="2026-01-02T10:00:00Z",text="请先选择会话，再同步可访问消息到离线检索索引。")))
