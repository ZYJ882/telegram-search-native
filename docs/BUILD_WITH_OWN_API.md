...
应用在每次初始化 TDLib 时，按以下顺序选择参数：

| 优先级 | 配置来源 | 行为 |
|---|---|---|
| 1 | 设备内手动保存的 API | 只要用户在“设置”中保存了有效格式的参数，就优先使用这组加密保存的自定义 API。 |
| 2 | 构建时默认 API | 仅在设备内没有自定义 API 时使用 `TG_DEFAULT_API_ID` 与 `TG_DEFAULT_API_HASH`。 |
| 3 | 无可用配置 | 跳转到设置页，提示填写自己的 API。 |

如果 Telegram 在 TDLib 初始化阶段拒绝当前 API ID 或 API Hash，应用会明确提示当前失败的是“设备内自定义 API”还是“构建默认 API”，并给出下一步处理方式。为避免隐藏错误配置，**自定义 API 失败时不会静默自动改用构建默认 API**；使用者可以核对并重新保存参数，或在存在构建默认 API 的版本中，使用设置页“清除自定义 API，恢复构建默认 API”操作后重试。
